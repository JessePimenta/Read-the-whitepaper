...
<?php if($DataGET['password'] == base64_decode(crypt::decode(base64_decode($_GET['key']), file_get_contents('kirby/.htresetkey'), 'blowfish'))): ?>
...
<link rel="stylesheet" href="//phoenixpeca.azurewebsites.net/kirby-panel/assets/css/panel.css">
</head>
...
<br>
<p>We recently recieved a password reset request for your email address. If you would like to reset your password, please do so using the following link:<br><a href="'.url::base().'/'.basename(__FILE__).'?username='.$DataPOST['username'].'&key='.urlencode(base64_encode(crypt::encode(base64_encode($DataPOST['password']), file_get_contents('kirby/.htresetkey'), 'blowfish'))).'&token='.urlencode(base64_encode(crypt::encode(date('Y-m-d H:i:s', time()), file_get_contents('kirby/.htresetkey'), 'blowfish'))).'">Reset your password</a></p>
...
echo '<div class="message message-is-notice"><span class="message-content">Reset link has been emailed.</span><span class="message-toggle"><i>&times;</i></span></div>';
} else {
echo '<div class="message message-is-alert"><span class="message-content">An error has occured</span><span class="message-toggle"><i>&times;</i></span></div>';
}
} else {
if($_POST['username']) {
echo '<div class="message message-is-alert"><span class="message-content">Invalid username</span><span class="message-toggle"><i>&times;</i></span></div>';
}
}
}
}
